export const Min = (limit: number, validateOnly: boolean) => {
  return (target: any, propertyName: string) => {
    ...
    Object.defineProperty(target, propertyName, {
      ...
      set: (newVal: number) => {
        ...
        if (newVal < limit) {
          // throw new Error(`Field ${propertyName} must be bigger than ${limit}!`);
        }
      }
    });
    ...
    Object.defineProperty(target, errorKey, {
      get: function () {
        return value && value < limit ? `Field ${propertyName} must be bigger than ${limit}` : '';
      },
      enumerable: true,
      configurable: true
    });
